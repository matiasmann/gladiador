...
<?php
		...
		if ( 1 === $comment_count ) {
			...
			printf(
				...
				esc_html__( 'One thought on &ldquo;%1$s&rdquo;', 'gladiador' ),
...
?>
			...
			<a href="#respond">
				<span class="screen-reader-text"><?php esc_html_e( 'Leave a comment', 'gladiador' ); ?></span>
				<?php esc_html_e( '&rsaquo;', 'gladiador' ); ?>
			</a>
...
<?php
		...
		wp_list_comments(
			array(
				'style' => 'ol',
				/**
				 * The filter `gladiador_comments_avatar_size` allows you to
				 * change the size of the avatars in the comments.
				 */
				'avatar_size' => apply_filters( 'gladiador_comments_avatar_size', 80 ),
				'short_ping' => true,
			)
		);
	...
	if ( comments_open() ) {
		...
		comment_form(
			array(
				...
				'cancel_reply_before' => '',
				'cancel_reply_after' => '',
				'cancel_reply_link' => esc_html__( 'Cancel Reply', 'gladiador' ),
			)
		);
...
?>
		...
		<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'gladiador' ); ?></p>
