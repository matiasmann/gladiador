 ...
 *
 * The time is generated by {@see gladiador_human_time_diff}.
 */
function gladiador_post_time() {
 ...
 */
function gladiador_post_author() {
...
/**
 * Display a link to the Gladiador Comments.
 *
 * Used in post meta, and adds a smooth scroll effect to add context to the
 * comments position.
 */
function gladiador_comments_link() {
	...
	comments_popup_link(
		sprintf(
			wp_kses(
				...
				__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'gladiador' ),
				array(
					'span' => array(
						'class' => array(),
					),
 ...
 * 'read more'.
 */
function gladiador_read_more_text() {

	// Get post data.
	$post = get_post();
	$custom_readmore = array(
		'more_text' => esc_html__( 'Read more', 'gladiador' ),
	);
	...
	if ( ! $post_title ) {
		...
		esc_html_e( 'Read more', 'gladiador' );
		return;
 ...
 * @return WP_Query
 */
function gladiador_child_pages() {
 ...
 */
function gladiador_contributor( $user_id = null, $post_count = null ) {

	if ( ! get_theme_mod( 'gladiador_single_show_author_details', true ) && ! is_customize_preview() ) {
		return false;
	}
...
<?php
		...
		printf( esc_html( _n( '%d Article', '%d Articles', (int) $post_count, 'gladiador' ) ), (int) $post_count );
?>
			</a>
		</p>
 ...
 */
function gladiador_project_terms() {
...
?>
	<p class="projects-terms entry-meta">
		...
		<span class="project-terms-intro">
			<?php esc_html_e( 'Categories:', 'gladiador' ); ?>
		</span>

		<a class="<?php echo esc_attr( $class ); ?>" href="<?php echo esc_url( home_url( '/portfolio/' ) ); ?>"><?php esc_html_e( 'All', 'gladiador' ); ?></a>
 ...
 * preview.
 */
function gladiador_archive_image() {

	$archive_articles = (int) get_theme_mod( 'gladiador_archive_articles', '0' );
	...
	return get_the_post_thumbnail( (int) get_the_ID(), 'gladiador-archive' );
 ...
 */
function gladiador_credits_content( $wrapper = true ) {

	/**
	 * Contents is not escaped here. It is already escaped in the
	 * gladiador_credits_get_content function.
	 */
	$contents = gladiador_credits_get_content();
 ...
 */
function gladiador_credits_get_content() {

	$default = '(privacy)(|)(ptd)(|)(top)';
	$separator = '<span role="separator" aria-hidden="true" class="sep"></span>';
	$top_link = '<a href="#header">' . esc_html__( 'Top', 'gladiador' ) . '</a>';
	/* Translators: %1$s = theme name, %2$s = theme author website */
	$pro_theme_link = sprintf( esc_html__( 'Theme: %1$s by %2$s', 'gladiador' ), 'Gladiador', '<a href="https://github.com/matiasmann" rel="nofollow">Matias Mann</a>' );

	/**
	 * The theme mod is escaped when the function returns its value.
	 */
	$contents = get_theme_mod( 'gladiador_credits_content', $default );
	...
	$contents = str_ireplace( '(FEED)', '<a href="' . get_feed_link() . '">' . esc_html__( 'RSS Feed', 'gladiador' ) . '</a>', $contents );
	$contents = str_ireplace( '(PRIVACY)', get_the_privacy_policy_link(), $contents );
	$contents = str_ireplace( '(PTD)', $pro_theme_link, $contents );

	$contents = apply_filters( 'gladiador_footer_content', $contents );
 ...
 */
function gladiador_breadcrumbs() {
 ...
 */
function gladiador_social_menu() {
 ...
 */
function gladiador_print_css( $stylesheet ) {
	...
	// Output the styles.
	echo '<style id="gladiador-partial-' . esc_attr( $slug ) . '">';
 ...
 */
function gladiador_print_plugin_css( $plugin ) {

	gladiador_print_css( 'assets/css/plugin-' . $plugin );
