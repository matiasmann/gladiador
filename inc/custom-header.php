 ...
 */
function gladiador_custom_header_support() {

	add_theme_support(
		'custom-header',
		apply_filters(
			'gladiador_custom_header',
			array(
...
add_action( 'after_setup_theme', 'gladiador_custom_header_support' );
 ...
 */
function gladiador_colour_brightness( $color = '', $lighter_than = 130 ) {
 ...
 * images are found.
 */
function gladiador_header() {

	$header_image = get_header_image();

	// Use custom headers on singular pages, but only if the image is large
	// enough.
	if ( is_singular() && get_theme_mod( 'gladiador_single_header', false ) ) {

		/**
		 * Use get_queried_object_id so that the content id will always be found
		 * in cases where $post has not been set.
		 */
		$image = wp_get_attachment_image_src( (int) get_post_thumbnail_id( get_queried_object_id() ), 'gladiador-header' );
 ...
 */
function gladiador_has_header_image() {
 ...
 */
function gladiador_header_height() {

	$header_height = 0;

	if ( is_singular() ) {

		$header_height = get_theme_mod( 'gladiador_single_header_height', '1' );

	} else {

		$header_height = get_theme_mod( 'gladiador_archive_header_height', '1' );
