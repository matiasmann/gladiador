...
function gladiador_get_fonts() {
	...
	return apply_filters( 'gladiador_fonts', $fonts );
 ...
 * @uses inc/customizer/fonts.php
 */
function gladiador_get_font_css() {

	$fonts = gladiador_get_fonts();

	$styles = array();

	$styles[] = 'body {';

	$styles[] = '--font-body:' . $fonts[ get_theme_mod( 'gladiador_body_font', 'cambria' ) ][1] . ';';
	$styles[] = '--font-title:' . $fonts[ get_theme_mod( 'gladiador_title_font', 'cambria' ) ][1] . ';';
	$styles[] = '--font-header:' . $fonts[ get_theme_mod( 'gladiador_header_font', 'cambria' ) ][1] . ';';
	$styles[] = '--font-meta:' . $fonts[ get_theme_mod( 'gladiador_meta_font', 'cambria' ) ][1] . ';';
 ...
 * @uses inc/customizer/fonts.php
 */
function gladiador_get_font_json() {

	$fonts = gladiador_get_fonts();

	return 'gladiador_fonts = ' . wp_json_encode( $fonts ) . ';';
 ...
 */
function gladiador_get_single_css() {
	...
	if ( ! get_theme_mod( 'gladiador_single_show_author', true ) ) {
		$styles[] = 'html .byline { display: none; }';
	}

	if ( ! get_theme_mod( 'gladiador_single_show_date', true ) ) {
		$styles[] = 'html .posted-on { display: none; }';
	}

	if ( ! get_theme_mod( 'gladiador_single_show_categories', true ) ) {
 ...
 * @return string
 */
function gladiador_get_colour_css() {

	$styles = array();

	$colour_light = gladiador_get_background_color();
	...
	if ( get_theme_mod( 'gladiador_dark_mode', false ) ) {
		...
		$colour_dark = get_theme_mod( 'gladiador_dark_mode_colour', '#004466' );

	}

	$black = apply_filters( 'gladiador_colour_black', '#000' );
	$white = apply_filters( 'gladiador_colour_white', 'rgba(255,255,255,0.9)' );
	...
	$styles[] = '--foreground-color-light:' . esc_attr( gladiador_colour_brightness( $colour_light ) ? $black : $white ) . ';';
	$styles[] = '--foreground-color-dark:' . esc_attr( gladiador_colour_brightness( $colour_dark ) ? $black : $white ) . ';';

	$styles[] = '--foreground-contrast-color-light:' . esc_attr( gladiador_colour_brightness( $colour_light ) ? $white : $black ) . ';';
	$styles[] = '--foreground-contrast-color-dark:' . esc_attr( gladiador_colour_brightness( $colour_dark ) ? $white : $black ) . ';';
 ...
 */
function gladiador_title_styles() {

	$header_visibility = (int) get_theme_mod( 'gladiador_site_title', '0' );
	$styles = array();
