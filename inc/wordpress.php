 ...
 */
function gladiador_enqueue() {

	// Scripts.
	wp_enqueue_script(
		'gladiador-script-global',
		gladiador_get_script_file(),
		array(),
		gladiador_get_theme_version( '/assets/scripts/global.js' ),
		true
	);

	wp_script_add_data( 'gladiador-script-global', 'script_execution', 'async' );
...
add_action( 'wp_enqueue_scripts', 'gladiador_enqueue' );
 ...
 */
function gladiador_print_styles() {

	gladiador_print_css( 'style' );

	/**
	 * No escaping needed.
	 * The gladiador_get_site_styles function generates, and escapes
	 * everything neccessary.
	 */
	echo '<style id="gladiador-custom-styles">' . gladiador_get_site_styles() . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

}

add_action( 'wp_print_styles', 'gladiador_print_styles' );
 ...
 * @return string
 */
function gladiador_on_style_load( $html, $handle ) {

	if ( 'gladiador-style' === $handle ) {
...
add_filter( 'style_loader_tag', 'gladiador_on_style_load', 10, 2 );


/**
 * Enqueue WordPress theme styles within Gutenberg.
 */
function gladiador_editor_blocks_styles() {
	 ...
	 */
	wp_enqueue_style(
		'gladiador-editor-blocks',
		get_theme_file_uri( '/assets/css/editor-blocks.css' ),
		array(),
		gladiador_get_theme_version( '/assets/css/editor-blocks.css' )
	);

	// Add custom properties for the block editor.
	wp_add_inline_style( 'gladiador-editor-blocks', gladiador_get_block_styles() );
	 ...
	 */
	if ( ! gladiador_colour_brightness( gladiador_get_background_color() ) ) {
...
add_action( 'enqueue_block_editor_assets', 'gladiador_editor_blocks_styles' );


/**
 * Get the custom properties for the site so that we can override them.
 */
function gladiador_get_background_color() {

	return get_theme_mod( 'gladiador_light_mode_colour', '#eedd33' );
 ...
 */
function gladiador_get_block_styles() {

	$background_colour = gladiador_get_background_color();
 ...
 */
function gladiador_get_site_styles() {

	$styles = array();

	$styles[] = gladiador_title_styles();
	$styles[] = gladiador_get_font_css();
	$styles[] = gladiador_get_single_css();
	$styles[] = gladiador_get_colour_css();
	$styles[] = gladiador_header();
 ...
 */
function gladiador_content_width() {

	$width = 900;

	$GLOBALS['content_width'] = apply_filters( 'gladiador_content_width', $width );

}

add_action( 'template_redirect', 'gladiador_content_width', 0 );


/**
 * Set up all the theme properties and extras.
 */
function gladiador_after_setup_theme() {

	/**
	 * Setup theme translations.
	 *
	 * Translations can be found in the wp-content/themes/gladiador/languages/
	 * directory.
	 */
	load_theme_textdomain( 'gladiador', get_parent_theme_file_path( '/languages' ) );
	...
	add_theme_support(
		...
		array(
			array(
				'name' => esc_html__( 'White', 'gladiador' ),
				'slug' => 'primary',
				'color' => '#ffffff',
			),
			array(
				'name' => esc_html__( 'Light Gray', 'gladiador' ),
				'slug' => 'secondary',
				'color' => '#f5f5f5',
			),
			array(
				'name' => esc_html__( 'Black', 'gladiador' ),
				'slug' => 'highlight',
				'color' => '#000000',
			),
		)
	 ...
	 * HTML5 FTW.
	 *
	 * This does not include support for the search-form since Gladiador has its
	 * own form layout.
	 *
	 * @see searchform.php
	 *
	 ...
	 */
	register_nav_menu( 'menu-1', esc_html__( 'Menu', 'gladiador' ) );
...
add_action( 'after_setup_theme', 'gladiador_after_setup_theme' );


/**
 * Include the default site header for the site.
 */
function gladiador_include_header() {

	get_template_part( 'parts/header' );

}

add_action( 'gladiador_header', 'gladiador_include_header' );


/**
 * Include the default site footer for the site.
 */
function gladiador_include_footer() {

	get_template_part( 'parts/footer' );

}

add_action( 'gladiador_footer', 'gladiador_include_footer' );
 ...
 */
function gladiador_widgets_init() {

	// Footer Widgets.
	register_sidebar(
		array(
			'name' => esc_html__( 'Footer Widgets', 'gladiador' ),
			'id' => 'sidebar-1',
			'description' => esc_html__( 'Widgets that display at the bottom of your website. They are arranged in rows.', 'gladiador' ),
...
add_action( 'widgets_init', 'gladiador_widgets_init' );
 ...
 * @return int
 */
function gladiador_excerpt_length( $length ) {

	return 30;

}

add_filter( 'excerpt_length', 'gladiador_excerpt_length', 999 );
 ...
 * @return string
 */
function gladiador_excerpt_more() {

	return ' &hellip; ';

}

add_filter( 'excerpt_more', 'gladiador_excerpt_more' );
 ...
 */
function gladiador_post_terms( $content = '' ) {
	...
	$categories_list = get_the_category_list( esc_html__( ', ', 'gladiador' ) );
	if ( $categories_list ) {
		...
		$terms .= sprintf( '<p class="taxonomy entry-meta tax-categories">' . esc_html__( 'Posted in: %1$s', 'gladiador' ) . '</p>', $categories_list ); // WPCS: XSS OK.

	}

	/* translators: used between list items, there is a space after the comma */
	$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'gladiador' ) );

	if ( $tags_list && ! is_wp_error( $tags_list ) ) {

		/* translators: %1$s will be replaced with a list of tags */
		$terms .= sprintf( '<p class="taxonomy entry-meta tax-tags">' . esc_html__( 'Tagged as: %1$s', 'gladiador' ) . '</p>', $tags_list ); // WPCS: XSS OK.
...
add_filter( 'the_content', 'gladiador_post_terms' );
 ...
 */
function gladiador_wrap_the_archive_title( $title ) {
...
add_filter( 'get_the_archive_title', 'gladiador_wrap_the_archive_title' );
 ...
 * @return string
 */
function gladiador_category_list_span( $cat_list ) {
...
add_filter( 'the_category', 'gladiador_category_list_span' );
add_filter( 'the_tags', 'gladiador_category_list_span' );
 ...
 * @return string
 */
function gladiador_change_menu( $menu_html = '' ) {
...
add_filter( 'wp_page_menu', 'gladiador_change_menu' );
 ...
 */
function gladiador_theme_colour() {

	// Use the user defined background colour.
	$colour = gladiador_get_background_color();

	if ( get_theme_mod( 'gladiador_dark_mode', false ) ) {

		$colour = get_theme_mod( 'gladiador_dark_mode_colour', '#004466' );
...
add_filter( 'wp_head', 'gladiador_theme_colour' );
 ...
 */
function gladiador_link_pages_link( $html ) {
...
add_filter( 'wp_link_pages_link', 'gladiador_link_pages_link' );


/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function gladiador_pingback_header() {
...
add_action( 'wp_head', 'gladiador_pingback_header' );
 ...
 */
function gladiador_widget_tag_cloud_args( $args ) {
...
add_filter( 'widget_tag_cloud_args', 'gladiador_widget_tag_cloud_args' );
 ...
 * @return string
 */
function gladiador_get_theme_version( $filepath = '' ) {
 ...
 * @return string
 */
function gladiador_get_script_file() {
 ...
 * @return string
 */
function gladiador_filter_script_loader_tag( $tag = '', $handle ) {
...
add_filter( 'script_loader_tag', 'gladiador_filter_script_loader_tag', 10, 2 );


/**
 * Register the required plugins for this theme.
 */
function gladiador_register_required_plugins() {
...
add_action( 'tgmpa_register', 'gladiador_register_required_plugins' );
